<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('.item-detail-key li').click(function () {
            if ($(this).hasClass('key-select')) {
                $(this).removeClass('key-select');
            } else {
                $(this).parent().children().removeClass('key-select');
                $(this).addClass('key-select');
            }
        });
    });
</script>
...
<div class="col-span-8">
    ...
    <div class="item-detail-meta">
        ...
        <ul>
            <li class="price1">市场价：
                <del><?php echo $model->currency . $model->market_price ?></del>
            </li>
    ...
    <div class="item-detail-key">
        <dl>
            ...
            foreach ($new_arr as $k => $v) {
                ...
                echo '<dt>' . $list->prop_name . ': </dt><dd><ul class="item-detail-key">';
                foreach ($v as $v) {
                    $v_list = PropValue::model()->findByPk($v);
                    echo '<li><a>' . $v_list->value_name . '</a></li>';
                }
                echo '</ul></dd>';
            }
            ?>
            <dt>我要订购：</dt>
            <dd><input type="number" min="1"
                       value="<?php echo $model->min_number ?>"
                       name="qty" class='background-color-4 input-small'
                       style='border-color:#e66f69;color:#fff;margin-top:3px;padding: 0;float: left;width: 100px;'></dd>
            </dt>
        </dl>
    </div>
